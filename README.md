...
Dual Finder 纪 是一个 macOS 双栏文件管理器，把 **Finder 的系统集成**和 **Total Commander 风格的左右目录操作**结合起来。基于 SwiftUI + AppKit，核心逻辑在 `DualFinderCore`，界面在 `DualFinderApp`。

## 为什么选择 Dual Finder 纪

| 优势 | 说明 |
| --- | --- |
| **真正的双栏工作流** | 左右独立 Tab、历史、选择；复制/移动/Sync/Mirror 一键跨栏，不必在两个 Finder 窗口间来回切 |
| **大目录也敢用** | 递归扫描缓存、Sync 跳过相同文件（大小+mtime）、进度显示文件数/字节/skipped/copied；适合 TB 级文件夹补全 |
...
- **Sync to Other Pane**：跳过目标侧已存在且相同的文件，只复制缺失或不同的项（适合大文件夹增量补全）。
- **Mirror to Other Pane**：在 Sync 基础上删除目标侧来源中不存在的文件；操作前显示将删除的文件数与总大小。

### 文件操作

- 左右栏之间复制选择项。
- 左右栏之间移动选择项。
- 左右栏之间 **Sync** / **Mirror** 同步（见上）。
- 文件复制、移动和移到废纸篓进入操作队列，底部显示最近运行/排队任务、进度和当前项目。
- 大文件夹操作进度显示：**根 item 数**、**文件数 x/y**、**字节进度**、**skipped/copied 数量与大小**、耗时。
...
- 右键菜单包含：
  ...
  - 复制绝对路径、压缩/解压、加入收藏、打开终端、批量重命名、复制/移动/**Sync**/**Mirror** 到另一栏、移到废纸篓。
- 可打开日志目录。
...
- 定时 `[memory] diagnostics` 日志（footprint、RSS、缓存条目、队列状态），便于排查内存问题。
- 状态栏每 5 秒显示进程内存；RSS 明显高于 footprint 时一并展示。
...
| `Command-Shift-N` | 在当前活动栏新建文件夹并进入命名 |
...
| P2 | 更可信的目录同步 | 批量同步、hash 校验 | 已有 Sync、Mirror（含删除预览）、递归对比和单项复制；缺少 hash 校验与批量计划任务 |
